...
const CreateBlog = () => {
  ...
  const handleCreateBlog = async (e) => {
    ...
    try {
      const { data } = await axios.post("http://localhost:4001/api/blogs/create",
        formData,
        {
        ...
        <div className=' p-4 min-h-screen flex items-center justify-center bg-gray-100 '>
          <div className='md:w-[50%] p-8 shadow-lg bg-white rounded-md'>
            <form onSubmit={handleCreateBlog}>
              ...
              <select onChange={(e) => setCategory(e.target.value)} value={category} className='w-full p-2 rounded-md mb-4 border'>
              ...
              <div className='mb-4'>
                <input onChange={(e) => setTitle(e.target.value)} type="text" placeholder='Enter Title' value={title} className='w-full p-2 rounded-md border' />
              </div>
              ...
              <button type='submit' className='w-full bg-blue-500 p-2 hover:bg-blue-700 duration-300 rounded-md  text-white'>Post Blog</button>
