...
const MyBlogs = () => {
  ...
  useEffect(() =>  {
    const fetchMyBlogs = async () => {
      try {
        const { data } = await axios.get('https://blog-app-xygc.onrender.com/api/blogs/my-blog', { withCredentials: true });
        console.log(data);
        SetmyBlogs(data);
  ...
  const handleDelete=async(id)=>{
    await axios.delete(`https://blog-app-xygc.onrender.com/api/blogs/delete/${id}`, {withCredentials:true})
    .then((res)=>{
...
console.log(myBlogs)
  return (
    <>
      ...
      <div className='grid  grid-cols-1 sm:grid-cols-2  md:grid-cols-4 gap-5 w-[50%] place-content-center'>
        {myBlogs.map((element) => {
          return (
            <div key={element._id} className='border-2 border-black'>
              <div>
